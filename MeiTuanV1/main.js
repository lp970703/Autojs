/**美团快消应用V1：该功能通过pc端的excel，利用excel自带的插件==>excel to json将
 * excel转为json文件，将json文件放到手机路径下，通过对该数据的抓取（既入参），根据
 * 这些数据，在美团上通过修改地址，找到指定超市，将超市的商品数据爬虫下来，并保存至
 * json格式，放到手机目录底下，用户找到手机目录下的输出json，再利用pc端excel自带的
 * ==>json to excel将数据转为excel。
 */
...
function findStore() {
    ...
    // for(let index = 1; index < paramsData.length; index++) {
        ...
        selectAddress(3); sleep(2000);
        // 2、选择具体的store名称，并找到全部商品
        const depth = selectStore(3); sleep(2000); // 这里的等待是因为有点击全部商品，有加载的过程
...
function selectAddress(index) {
    ...
    className("android.view.View").depth(19).desc("外卖").findOne().click();
...
function findItemGoodsData() {
    for(let x=0; x < 10; x++) {
        console.log("第："+x+"个");
        if (id("txt_stickyfoodList_adapter_food_name").find()[x]) {
            // 1、点进具体的商品
            let b = {};
            console.log(id("txt_stickyfoodList_adapter_food_name").find()[x].text());
            b.name = id("txt_stickyfoodList_adapter_food_name").find()[x].text();
            console.log(id("tv_stickyfood_sold_count").find()[x].text());
            b.sale = id("tv_stickyfood_sold_count").find()[x].text();
            res.push(b);
            sleep(1000);
            console.log(res);
        }
