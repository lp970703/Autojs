...
stopApp("抖音");
//更新注销
files.remove("/sdcard/飞瓜筛选导出数据.json");

launchApp("网易邮箱");sleep(8000);
while(text("邮件").exists() == false && text("文件").exists() == false && text("任务").exists() == false){
    back();sleep(1200);
}
new CyclonePublic(text,"邮件",false,false,"click",0).elemClick();sleep(1500);
new CyclonePublic(id,"com.netease.mobimail:id/iv_search",false,false,"click",0).elemClick();sleep(1500);
setText("飞瓜筛选导出数据.json");sleep(1500);
new CyclonePublic(id,"com.netease.mobimail:id/tv_sender",false,false,"click",0).elemClick();sleep(1500);
new CyclonePublic(id,"com.netease.mobimail:id/iv_save",false,false,"click",0).elemClick();sleep(1500); 
new CyclonePublic(id,"com.netease.mobimail:id/tv_title",false,false,"click",0).elemClick();sleep(1500);
new CyclonePublic(desc,"更多选项",false,false,"click",0).elemClick();sleep(1500);
if(text("隐藏内部存储设备").exists() == false){
    new CyclonePublic(desc,"显示内部存储设备",false,false,"click",0).elemClick();sleep(1500);
}else{
    back();sleep(1200);
}
new CyclonePublic(desc,"显示根目录",false,false,"click",0).elemClick();sleep(1500);

// new CyclonePublic(text,"OnePlus 8",false,false,"click",0).elemClick();sleep(1500);
// id("com.google.android.documentsui:id/roots_list").find()[0].child(2).click();sleep(1500);
//更新注销
new CyclonePublic(text,"HRY-AL00T",false,false,"click",0).elemClick();sleep(1500);
new CyclonePublic(text,"保存",false,false,"click",0).elemClick();sleep(3000);
data = files.read("/sdcard/飞瓜筛选导出数据.json");
sleep(1200);files.remove("/sdcard/飞瓜筛选导出数据.json");
data = JSON.parse(data) 
// var data = {
//     username : ["linyy0810","Supergirl5721","fuxiaoyu3698"]
// }
...
for(j of data.username){
    try{
        ...
        if(id("com.ss.android.ugc.aweme:id/ln1").exists() == true){
            try{
                new CyclonePublic(id,"com.ss.android.ugc.aweme:id/ln1",false,false,"click",0).elemClick();sleep(2500);
            }catch(error){
                while(text("请完成下列验证后继续:").exists() == true){
                    start()
                }
